...
class FigureCriticService(figure_critic_pb2_grpc.FigureCriticServiceServicer):
    """
    A class implementing FigureCriticService
    """
    ...
    def AssessArticleFigures(self, request, context):
        # Extract input from request
        pdf_content = request.pdf
        title = request.title
        abstract = request.abstract

        figure_assessment = self.analyze_pdf(
            pdf_content=pdf_content,
            title=title,
            abstract=abstract
        )

        return figure_critic_pb2.figureAnalysisResult(
            success=True,
            assessment=figure_assessment
        )
