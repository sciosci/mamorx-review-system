...
class FigureCriticClient(object):
    def __init__(self, api_config: APIConfigs):
        self.channel = grpc.insecure_channel(
            api_config["figure_critic_url"],
            options=[
                ("grpc.max_send_message_length", 20000000),
                ('grpc.max_receive_message_length', 20000000)
            ]
        )
        self.stub = figure_critic_pb2_grpc.FigureCriticServiceStub(self.channel)


    def critic_pdf_file(self, pdf_file_path: str, title: str, abstract: str) -> str:
        # Open and load PDF file and bytes
        pdf_file_bytes = open(pdf_file_path, "rb").read()

        # Create Request Payload
        request_payload = figure_critic_pb2.articleToAnalyze(
            pdf=pdf_file_bytes,
            title=title,
            abstract=abstract
        )
        ...
        result = self.stub.AssessArticleFigures(request_payload)

        return result.assessment

    # TODO: Look into how to close a gRPC channel when the object is deleted 
    # def __del__(self):
    #     self.channel.close()
